...
async function submitImage2VideoTask(params: {
  ...
  const response = await fetch("https://app-bs8umh6bsikh-api-eLMlJj3KJD89.gateway.appmedo.com/v1/videos/image2video", {
    method: "POST",
    headers: {
...
async function pollUntilDone(taskId: string): Promise<{ id: string; url: string; duration: string }[]> {
  ...
  while (Date.now() < deadline) {
    ...
    const res = await fetch(
      `https://app-bs8umh6bsikh-api-rLobzpqX85m9.gateway.appmedo.com/v1/videos/image2video/${taskId}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
